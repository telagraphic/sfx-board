 ...
 * 
 * The class uses the custom Sound class for reliable audio playback
...
export default class SoundBoard {
     ...
     */
    constructor() {
        this.sounds = {};                    // Cache of loaded Sound objects
    ...
    /**
     * Load a single sound on-demand using the custom Sound class
     * 
     * This method implements lazy loading - audio files are only downloaded
     * when users actually want to play them. It creates a Sound instance
     * that wraps the HTML5 Audio API with a clean interface.
     * 
     * The method:
     * 1. Checks if sound is already loaded (returns cached version)
     * 2. Validates the audio URL is accessible
     * 3. Creates a new Sound instance
     * 4. Caches the result for future use
     * 
     * @param {string} clipName - The name of the sound clip to load
     * @returns {Promise<Sound>} Promise that resolves to a Sound instance
    ...
    async loadSound(clipName) {
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                const sound = new Sound(clip.file, clipName);
                
                // Set up ended callback for state management
                sound.on('end', () => {
                    if (!sound.loop()) {
